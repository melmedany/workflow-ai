...
└── domain
    ...
    └── workflow                        stage contract, state, graph assembly, policy, events, executor
        └── response                    response contract, format, validation result
```
...
src/test
├── java/io/workflowai
│   ├── application/…                   stage-level unit tests
...
neither Spring nor LangChain4j; the application layer depends on ports rather than adapters and on neither AI framework;
LangChain4j allowed only under `adapter.out.chat`; LangGraph4j only under `domain.workflow`; and
`adapter.in` and `adapter.out` never reference each other.
...
or `./gradlew build` to compile and test together.
...
- Unit tests under `application/execution/stage` cover classification's scheduling behaviour, `CREATE_TASK`'s clarify
  and refuse branches, and the remaining stages without Spring or a database.
